...
if( ! class_exists('acf_custom_field_relationship') ) :

class acf_custom_field_relationship extends acf_field {
        ...
        */
        ...
        public static function imageshare_get_post_status_label($post_status) {
            return get_post_statuses()[$post_status];
        }

        public static function imageshare_get_post_statuses() {
            return array_keys(get_post_statuses());
        }

        public static function imageshare_get_pretty_post_statuses($post_statuses = array()) {
            // get post statuses
        ...
        function initialize() {
                ...
                // vars
                $this->name = 'imgs_relationship';
                $this->label = __("Relationship (Imageshare)",'imageshare');
                ...
                $this->defaults = array(
                        ...
                        'min'                           => 0,
                        'max'                           => 0,
                        'filters'                       => array('search', 'post_status', 'post_type', 'taxonomy'),
                        'elements'                      => array(),
                        'return_format'         => 'object'
                );

                // extra
                add_action('wp_ajax_acf/fields/imgs_relationship/query',                     array($this, 'ajax_query'));
                add_action('wp_ajax_nopriv_acf/fields/imgs_relationship/query',      array($this, 'ajax_query'));
        ...
        */
        ...
        function get_ajax_query( $options = array() ) {
                ...
                if( !empty($options['post_status']) ) {
                        ...
                        $args['post_status'] = self::imageshare_get_post_statuses();
        ...
        */
        ...
        function render_field( $field ) {
                ...
                $post_status = acf_get_array( $field['post_status'] );
                ...
                // post_status filter
                if( in_array('post_status', $filters) ) {

                        $filter_post_status_choices = array(
                                ''      => __('Select post status', 'imageshare')
                        ) + self::imageshare_get_pretty_post_statuses( $post_status );
                }
...
<div <?php acf_esc_attr_e($atts); ?>>
        ...
        <div class="filters -f<?php echo esc_attr($filter_count); ?>">
                ...
                /* post_status */
                if( in_array('post_status', $filters) ): ?>
                <div class="filter -post_status">
                        <?php acf_select_input( array('choices' => $filter_post_status_choices, 'data-filter' => 'post_status') ); ?>
                </div>
                <?php endif;
        ...
        */
        ...
        function render_field_settings( $field ) {
                ...
                // post_status
                acf_render_field_setting( $field, array(
                        'label'                 => __('Filter by Post Status','imageshare'),
                        'instructions'  => '',
                        'type'                  => 'select',
                        'name'                  => 'post_status',
                        'choices'               => self::imageshare_get_pretty_post_statuses(),
                        'multiple'              => 1,
                        'ui'                    => 1,
                        'allow_null'    => 1,
                        'placeholder'   => __("All post statuses",'imageshare'),
                ));
                ...
                acf_render_field_setting( $field, array(
                        ...
                        'choices'               => array(
                                ...
                                'post_status'           => __("Post Status",'imageshare'),
...
// initialize
acf_register_field_type( 'acf_custom_field_relationship' );
